...
public:
	...
	CPPUNIT_TEST( testAMTI12Channel_Reversed );
...
public:
	void testAMTI12Channel()
	{
		...
		// build model
		ForcePlate model;
		model.Type = ForcePlate::TypeAMTI;
		model.Model = "AccuGait/AccuSway";
		...
		bool setresult = calculator.SetModel(model, 1);
	...
	void testAMTI12Channel_Reversed()
	{
		// as above but reverse channel order as experienced on second of every pair of plates
		...
		// set force plate info - set as second plate in sequence which should trigger reverse channel order
		bool setresult = calculator.SetModel(model, 2);
		CPPUNIT_ASSERT_EQUAL(true, setresult);
